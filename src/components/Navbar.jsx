...
const Navbar = ({active,setActive}) => {
    ...
    if (currentPath === '/') {
      ...
      setActive('Q&A');
    }else if (/^\/sparepartdetails\/\d+$/.test(currentPath)) {
      setActive('sparepartdetaiils');
    }else if (currentPath === '/cart') {
      setActive('cart');
    }else if (currentPath === '/delivery') {
      setActive('delivery');
    }else if (/^\/rent\/\d+$/.test(currentPath)) {
      setActive('rent');
    }else if (currentPath === '/driverprofile') {
      setActive('Profile');
    }
  ...
  return (
      ...
      <div className="hidden sm:flex items-center space-x-20">
        <ul className="flex space-x-6 text-lg">
            ...
            <Link to='/cart' className='relative'>
              ...
              <span className="absolute -top-2 -right-2 w-[20px] h-[20px] flex justify-center items-center 
                  text-[14px] bg-red-700 text-white rounded-full">{cartCount}</span>
            </Link>
        </ul>
    ...
    {name==='Driver' && (<nav className='bg-black shadow-lg w-full transition-all duration-300 text-white py-6 fixed top-0 left-0 right-0 z-10 flex justify-between items-center px-24'>
      <Link to='/' onClick={()=>{setActive('Home')}}><h1 className="text-3xl font-bold ">Auto<span className='text-green-400'>Sphere</span></h1></Link>
      <div className="hidden sm:flex items-center space-x-20">
        <ul className="flex space-x-6 text-lg">
